...
import {VariableEditor} from '../variables/editor';
...
export function PropertyPanel() {
  ...
  const [currentWidget, setCurrentWidget] = useState<CanvasWidgetProps | null>(
    null
  );
  ...
  const handleWidgetNameChange = (value: string) => {
    console.log("🚀 ~ handleWidgetNameChange ~ value:", value)
    setWidgetName(value);
    if (selectedWidgetId) {
      updateWidget(selectedWidgetId, {name: value});
    }
  };
          ...
          <DialogContent className='sm:max-w-4xl h-[80vh] flex flex-col p-0'>
            ...
            <div className='flex-1 overflow-hidden p-4'>
              <CodeEditor
                height='60vh'
                initialValue={
                  currentCodeKey ? getJSCode(currentCodeKey) : undefined
                }
                onSave={handleJSSave}
              />
            </div>
